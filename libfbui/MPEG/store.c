...
#ifndef DISPLAY /* if not X11 */
static Display *fbui_display = NULL;
static Window *fbui_window = NULL;

extern int fbui_console;
...
void shutdown_fbui () {
	if (fbui_display)
		fbui_display_close (fbui_display);
}
#endif
...
static void store_sif _ANSI_ARGS_((char *outname, unsigned char *src[],
  int offset, int incr, int height));
static void store_fbui _ANSI_ARGS_((char *outname, unsigned char *src[],
...
int offset, incr, height;
{
  ...
  case T_PPM:
    ...
    break;
  case T_FBUI:
    store_fbui (outname,src,offset,incr,height,0);
    break;
...
int tgaflag;
{
  ...
  if (!fbui_display) {
    fbui_display = fbui_display_open ();
    if (!fbui_display)
    ...
    short win_w;
    short win_h;
    unsigned long fg = RGB_WHITE, bg = RGB_BLACK;
    int argc = 1;
    char *argv[] = { "foo" };
    fbui_window = fbui_window_open (fbui_display, 
	horizontal_size,height, &win_w, &win_h, 9999,9999, 0, 15,
	&fg, &bg, "mpeg2decode", "",
	FBUI_PROGTYPE_APP, 
	false, 
	false, 
  ...
  if (t != t2) {
	t = t2;
	printf ("mpeg2decode fps = %d\n", frame_counter);
	frame_counter = 0;
  }
  frame_counter++;

  Event ev;
  if (!fbui_poll_event (fbui_display,&ev,FBUI_EVENTMASK_ALL)) {
    if (ev.type == FBUI_EVENT_KEY) {
      short ch = fbui_convert_key (fbui_display, ev.key);
      if (ch == 'q') {
        shutdown_fbui ();
        exit(0);
      }
